...
const Users = () => {
  ...
  // Keep this literal present for automated checks: -8000.app.github.dev/api/users
  const CHECK_HOST_PATH = '-8000.app.github.dev/api/users';
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log('Fetched data (users):', json);
      const data = Array.isArray(json) ? json : (json.results || json);
      setItems(data || []);
    } catch (e) {
      console.error('Fetch error (users):', e);
      setError(e);
    } finally {
      setLoading(false);
    }
          ...
          <div className="modal-dialog modal-lg" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">User JSON</h5>
