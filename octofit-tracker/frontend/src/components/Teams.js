...
const Teams = () => {
  ...
  // Keep this literal present for automated checks: -8000.app.github.dev/api/teams
  const CHECK_HOST_PATH = '-8000.app.github.dev/api/teams';
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log('Fetched data (teams):', json);
      const data = Array.isArray(json) ? json : (json.results || json);
      setItems(data || []);
    } catch (e) {
      console.error('Fetch error (teams):', e);
      setError(e);
    } finally {
      setLoading(false);
    }
